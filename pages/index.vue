      ...
      .form-container.card(v-if="!userInfo.is_push")
        form()
          ...
          .section
            p 类型：
            div
              p(@click="selectType(GOODS_TYPE.NEW)" :class="{active: type === GOODS_TYPE.NEW}") 自营商家
              p(@click="selectType(GOODS_TYPE.OLD)" :class="{active: type === GOODS_TYPE.OLD}") 二手卖家
          .button(@click='submit')
            p 确定
</template>
