...
export const goodsResolver = {
  Goods: {
    ...
    seller: async (parent, args) => await getGoods({ foreign_id: args.id, type: config.goodsType.OLD}),
    shop: async (parent, args) => await getGoods({ foreign_id: args.id, type: config.goodsType.NEW})
  }
}
